...
it('should use attribute result set by checking function', function() {
    assert.equal(processHtml('<div b="/intel.html">Get in touch</div>',{
        allowedAttributes:function(nodeName,attribute){
            if (attribute.name === "b") attribute.value = "https://google.com";
            return attribute;
        }
...
it('should not escape inner content of script and style tags (when allowed)', function() {
    assert.equal(
        processHtml('<div>"normal text"</div><script>"this is code"</script>', {
            allowedTags: [ 'script' ],
            disableJavaScript:false
        }), '"normal text"<script>"this is code"</script>'
    );
    assert.equal(
        processHtml('<div>"normal text"</div><style>body { background-image: url("image.test"); }</style>', {
            allowedTags: [ 'style' ]
...
it('should not unescape escapes found inside script tags', function() {
    assert.equal(
        processHtml('<script>alert("&quot;This is cool but just ironically so I quoted it&quot;")</script>', { allowedTags: [ 'script' ], disableJavaScript:false }),
